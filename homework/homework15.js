...
/*Write a function named toSnakeCase() which takes a string as its argument and returns 
a new string in snake_case. Assume the string only contains letters and spaces
...
const toSnakeCase = (str) => str.trim().split(/\s+/).join('_').toLowerCase();
...
/*Write a function named alternatingCases() which takes a string argument and returns 
the string with alternating capitalization.
...
const alternatingCases = (str) =>
    ...
    str.trim().toLowerCase().split('').map((char, i) => {
...
const isNeutral = (str1, str2) => str1.split('').map((char, i) => char === str2[i] ? char : 0).join('');
